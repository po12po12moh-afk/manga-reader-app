...
import { manga, genres, mangaGenres, chapters, pages } from "./drizzle/schema";
...
async function seedData() {
  ...
  try {
    ...
    const insertedGenreIds: number[] = [];
    for (const genre of genreData) {
      const result = await db.insert(genres).values(genre);
      insertedGenreIds.push(Number(result[0].insertId));
    }
    ...
    const insertedMangaIds: number[] = [];
    for (const m of mangaData) {
      const result = await db.insert(manga).values(m);
      insertedMangaIds.push(Number(result[0].insertId));
    }

    // Assign genres to manga
    console.log("Assigning genres to manga...");
    const genreAssignments = [
      { mangaIndex: 0, genreIndices: [0, 2, 6] }, // Solo Leveling: Action, Fantasy, Supernatural
      { mangaIndex: 1, genreIndices: [0, 2, 6] }, // Tower of God: Action, Fantasy, Supernatural
      { mangaIndex: 2, genreIndices: [0, 2, 6] }, // TBATE: Action, Fantasy, Supernatural
      { mangaIndex: 3, genreIndices: [0, 2, 6] }, // ORV: Action, Fantasy, Supernatural
      { mangaIndex: 4, genreIndices: [0, 4, 7] }, // GOH: Action, Comedy, School Life
      { mangaIndex: 5, genreIndices: [0, 5, 6] }, // Noblesse: Action, Adventure, Supernatural
    ];

    for (const assignment of genreAssignments) {
      for (const genreIndex of assignment.genreIndices) {
        await db.insert(mangaGenres).values({
          mangaId: insertedMangaIds[assignment.mangaIndex],
          genreId: insertedGenreIds[genreIndex],
        });
      }
    }
